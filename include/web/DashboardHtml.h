...
<div class="status good"><span class="dot"></span>Wi-Fi AP</div>
...
<div class="actions"><button onclick="resetExtrema()">Reset min / max</button></div>
<footer><span id="uptime">Uptime --</span> · <span id="clients">0 Wi-Fi clients</span></footer>
</div>
<script>
...
async function refresh(){try{const r=await fetch('/api/telemetry',{cache:'no-store'});if(!r.ok)throw Error();const d=await r.json();metric('voltage',d.voltage,fmtV);metric('current',d.current,fmtI);metric('power',d.power,fmtP);metric('temperature',d.temperature,fmtT);metric('shuntVoltage',d.shuntVoltage,fmtS);status('sensor-status',d.sensorOK,d.sensorOK?'INA228 OK':'INA228 error');status('ble-status',d.bleConnected,d.bleConnected?'BLE connected':'BLE advertising');status('i2c-status',d.i2cErrors===0,'I2C errors: '+d.i2cErrors);status('display-status',d.displayOn,d.displayOn?'Display on':'Display off');document.getElementById('uptime').textContent='Uptime '+uptime(d.uptimeSeconds);document.getElementById('clients').textContent=d.wifiClients+(d.wifiClients===1?' Wi-Fi client':' Wi-Fi clients')}catch(e){status('sensor-status',false,'Connection lost')}}
